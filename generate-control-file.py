...
def parse_args():
    ...
    parser.add_argument("--output_dir", default="output_monet")
    parser.add_argument("--resolution", default=768, type=int, help="saved image resolution, <=768")
    parser.add_argument(
        "--pretrained_model_name_or_path",
        type=str,
        default="/home/zzj/.cache/huggingface/diffusers/models--runwayml--stable-diffusion-v1-5/snapshots/39593d5650112b4cc580433f6b0435385882d819",
...
if __name__ == "__main__":
    ...
    """
    for i in range(2650):
        original_image = load_image("comparison2//"+ str(i) +".jpg")
        ...
        for _ in tqdm(range(args.n_prompts)):
            ...
            for _ in range(num_forward_per_prompt):
                ...
                with torch.no_grad():
                    if args.reparam_seed is not None:
                        torch.manual_seed(0)
                    # x = pipe(image=original_image, control_image=canny_image,prompt_embeds=prompt_embs, num_images_per_prompt=args.bsz,controlnet_conditioning_scale=1.0, strength =0.8,guidance_scale=6.5)
                    x = pipe(image=original_image,control_image=canny_image,prompt_embeds=prompt_embs, num_images_per_prompt=args.bsz,controlnet_conditioning_scale=1.0, strength =0.8,guidance_scale=7.5)
                images = x.images
                for img in images:
                    ...
                    cur_i += 1
        # original_image = load_image("comparison/1.jpg")
